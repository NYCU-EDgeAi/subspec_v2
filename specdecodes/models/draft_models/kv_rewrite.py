...
class SubSpecSDDraftModel(ClassicSDDraftModel):
    ...
    def sim_run_postspec(self):
        for _ in range(self.draft_params.max_depth):
            if not self.postspec():
                break
    
    def get_draft_ids_after_post(self):
        """Return next draft block as [root, newly post-speculated tokens]."""
        token_ids = torch.cat(self.token_ids, dim=-1)
        # Include one boundary/root token so generator can keep using draft_ids[:, 1:].
        # Example: [..., t32, t33..t64] -> return [t32, t33..t64].
        return token_ids[:, -(self.postspec_count + 1):]
