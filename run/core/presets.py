...
def flashinfer_load_draft_model_v3(builder, target_model, tokenizer, draft_model_path):
    """Load draft model with FlashInfer v3 patches (based on working v1)."""
    try:
        from specdecodes.models.utils.flashinfer_v3.monkey_patch import apply_flashinfer_kernel_to_llama
    except ModuleNotFoundError as e:
        raise ImportError(
            f"Method '{builder.config.method}' requires the optional dependency 'flashinfer'.\n"
            "Hint: install it (and its deps), e.g. `pip install flashinfer-python`, then retry."
    ...
    # Apply FlashInfer v3 patches to both models (same as v1)
    apply_flashinfer_kernel_to_llama(attention=True, rms_norm=True, swiglu=False, model=target_model)
    apply_flashinfer_kernel_to_llama(attention=True, rms_norm=True, swiglu=False, model=draft_model)

    return draft_model

def flashinfer_load_kv_cache_v3(builder, target_model, draft_model):
    """Load KV cache for FlashInfer v3."""
    try:
        from specdecodes.models.utils.flashinfer_v3.cache_manager import FlashInferCache
    except ModuleNotFoundError as e:
        raise ImportError(
            f"Method '{builder.config.method}' requires the optional dependency 'flashinfer'.\n"
            "Hint: install it (and its deps), e.g. `pip install flashinfer-python`, then retry."
...
def register_presets():
    ...
    # SubSpec SD FlashInfer v3 (based on working v1 implementation)
    try:
        from specdecodes.helpers.recipes.subspec.hqq_4bit_postspec import (
            Recipe as SubSpecRecipeV1,
        )
        ...
        ModelRegistry.register(
            name="subspec_sd_fi3",
            generator_cls="specdecodes.models.generators.subspec_sd_fi3:SubSpecSDGenerator",
            draft_model_cls="specdecodes.models.draft_models.subspec_sd_fi3:SubSpecSDDraftModel",
            default_config={
                "llm_path": "meta-llama/Llama-3.1-8B-Instruct",
                "recipe": SubSpecRecipeV1(),
            },
            # TEST: Use v1's loaders to test if flashinfer_v3 modules are the issue
            load_draft_model_fn=flashinfer_load_draft_model,  # v1's loader
            load_kv_cache_fn=flashinfer_load_kv_cache,  # v1's cache loader
            needs_draft_kv_cache=False,
        )
    except ImportError:
        # If the base SubSpec recipe isn't importable, don't register this method.
